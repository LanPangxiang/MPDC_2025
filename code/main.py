...
from model import MPDC
from evaluation import eval_model
from pprint import pformat

def train_eval(model: MPDC, _all_ds):
    ...
    for epoch in range(gol.EPOCH):
        ...
        for idx, batch in enumerate(trn_loader):
            rec_loss, fisher_loss,cl_loss = model.getTrainLoss(batch)
            tot_loss = rec_loss + gol.conf['beta'] * fisher_loss + 0.1 * cl_loss
...
if __name__ == '__main__':
    ...
    recModel = MPDC(n_user, n_poi, geo_graph)
    if gol.LOAD:
