...
class SpaCn(MessagePassing):
    def __init__(self, in_channels, out_channels):
        super(SpaCn, self).__init__(aggr='add')
...
class BehCn(MessagePassing):
    def __init__(self, hid_dim, flow="source_to_target"):
        super(BehCn, self).__init__(aggr='add', flow=flow)
    ...
    def message(self, x_j, x_i, edge_index_j, edge_index_i, edge_l, edge_t, edge_size):
        prompt = torch.nn.Parameter(torch.Tensor(1,x_i.size(1))).to(gol.device)
